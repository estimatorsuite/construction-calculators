...
import {
  type LaminateFlooringState,
  type FlooringMaterial,
  MATERIAL_PRICES,
  CARTON_COVERAGE_SQFT,
  calculateLaminateFlooring,
  getDefaultState,
} from "../data/laminate-flooring";

const MATERIALS: FlooringMaterial[] = [
  "laminate",
  "vinylPlank",
  "engineeredHardwood",
  "solidHardwood",
...
export function LaminateFlooringCalculator() {
  const [state, setState] = useState<LaminateFlooringState>(getDefaultState());
  const result = calculateLaminateFlooring(state);

  function updateState(partial: Partial<LaminateFlooringState>) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            10% standard, 15% for L-shapes or diagonal layouts.
          </p>
        </div>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              updateState({ material: e.target.value as FlooringMaterial })
            }
            className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-base text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"
            style={{ minHeight: "44px", fontSize: "16px" }}
          >
            {MATERIALS.map((mat) => (
              <option key={mat} value={mat}>
                {MATERIAL_PRICES[mat].label} — ${MATERIAL_PRICES[mat].materialLow}-${MATERIAL_PRICES[mat].materialHigh}/sq ft
              </option>
            ))}
          </select>
        </div>
      ...
      {result ? (
        <div className="space-y-4">
            ...
            <div className="p-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="text-xl font-bold text-accent">
                  {result.roomAreaSqFt.toLocaleString()} sq ft
                </span>
              ...
              <div>
                ...
                <span className="text-xl font-bold text-foreground">
                  {result.cartonsNeeded}
                </span>
                <span className="text-xs text-muted-foreground block mt-0.5">
                  ({CARTON_COVERAGE_SQFT} sq ft/carton)
                </span>
              </div>
            </div>
          ...
          <div className="text-xs text-muted-foreground border-t border-border pt-2">
            <strong>Based on:</strong> {result.roomAreaSqFt.toLocaleString()} sq ft ×{" "}
            {state.wastePercent}% waste + {MATERIAL_PRICES[state.material].label}{" "}
            material. Prices from Angi + HomeAdvisor 2026 data. Last verified:
            June 2026.
          </div>
        </div>
      ) : (
