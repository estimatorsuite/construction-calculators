...
const JOB_TYPE_ICONS: Record<PlumbingJobType, React.ElementType> = {
  "water-heater-tank": Flame,
  "water-heater-tankless": Zap,
  "toilet-replace": Bath,
  "faucet-replace": Droplet,
};

const JOB_TYPES: PlumbingJobType[] = [
  "water-heater-tank",
...
export function PlumbingEstimateCalculator() {
  ...
  // 当前 jobType 是否需要许可（toilet/faucet 不需要）
  ...
  function handleJobTypeChange(job: PlumbingJobType) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Number of units (e.g., 2 toilets, 3 faucets)
          </p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              updateState({
                laborRatePerHour: Math.max(30, Number(e.target.value) || 85),
              })
            }
            className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"
            min={30}
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={() =>
              updateState({ includeEmergencyCall: !state.includeEmergencyCall })
            }
            className={`flex items-center gap-3 w-full rounded-lg border p-3 text-left transition-colors min-h-[44px] ${
          ...
          >
            ...
            >
              {state.includeEmergencyCall && <Check className="w-3 h-3" />}
            </div>
            <span className="text-sm text-foreground flex-1">
      ...
      <div className="text-xs text-muted-foreground bg-muted/30 rounded-lg p-3">
        <div className="flex items-start gap-2">
          ...
          <p>
            ...
            accessibility, and local labor rates. Gas line upgrades and
            repiping add significant cost not captured here. Always verify
            with your supplier.
          </p>
        </div>
      </div>
